...
本次定制功能回归检查发现三类行为与本仓库文档/默认配置的字面含义不一致，
全部为有意设计或已知限制，特此记录，避免部署时误判为故障：

1. **匿名读取被门禁覆盖**：`READ_ACCESS = 'ANONYMOUS'` 时，多空间门禁仍要求
   所有页面登录（`/`、`/Home`、`/sitemap.xml`、`/-/index` 均 302 到登录页，
   有测试固化）。该配置实际含义退化为「已登录用户无需额外审批即可读取」，
   `settings.cfg.skeleton` 与 `help_admin.md` 已按此修正说明。
2. **`PROXY_HEADER` 与多空间不兼容**：通过代理头登录的用户没有数据库账号，
   不参与空间授权，非管理员会静默 404；启动时仅打印告警。需要代理认证的
   部署应改用应用内账号体系或单空间模式。
3. **单进程假设**：维护门禁（`_readers`/`_process_locks`/`_site_maintenance`）
   是进程内状态，仅在 uwsgi 下校验单进程。gunicorn 多 worker 或多副本部署会
   让「请求排空」与维护互斥静默失效，仓库可能在其它 worker 读取时被替换；
   Git 同步与文档导入之间的互斥也会在同一实例内失效。请保持单 Web 进程形态。
